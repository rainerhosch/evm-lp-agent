/**
 * Uniswap V3 / PancakeSwap V3 position adapter (concentrated liquidity NFTs).
...
function npmContract(signerOrProvider) {
  return new ethers.Contract(config.dex.positionManagerAddress, npmAbi, signerOrProvider);
}

/**
 * Deploy concentrated liquidity (single-sided native preference in dry-run).
 * Live path: scaffold — prefers dry-run until approvals + tick math fully wired.
 */
export async function deployPosition({
  ...
  if (isDryRun()) {
    const fakeId = `dry-${config.chain.id}-${Date.now().toString(36)}`;
    ...
    log("deploy", `DRY RUN mint ${pos.pair} ${amount} ${config.chain.nativeSymbol}`);
    return {
      success: true,
      dry_run: true,
      position_id: fakeId,
      ...
      tick_lower: -half,
      tick_upper: half,
      message: "DRY RUN — no on-chain transaction sent",
    };
  }

  // Live mint requires token approvals, sorted token0/token1, sqrtPrice tick from pool slot0.
  // Scaffold returns explicit guidance rather than a half-broken tx.
  return {
    success: false,
    error:
      "Live mint is scaffolded. Set DRY_RUN=true for simulation, or extend tools/univ3.js with pool slot0 tick math + ERC20 approvals before enabling live mint.",
    hint: {
...
export async function getMyPositions({ force = false } = {}) {
  const tracked = getOpenPositions().filter((p) => p.chain === config.chain.id);

  // Enrich dry-run / tracked positions
  const positions = tracked.map((p) => {
    const age_minutes = p.deployed_at
      ? Math.floor((Date.now() - new Date(p.deployed_at).getTime()) / 60000)
      : null;
    // Without live pricing, dry-run PnL is 0 (fees not simulated yet)
  ...
  // Optionally list on-chain NPM NFTs (read-only)
  let onchain_count = null;
  if (!isDryRun() || force) {
    try {
      const wallet = getWallet();
      const npm = npmContract(getProvider());
      const bal = await npm.balanceOf(wallet.address);
...
export async function closePosition({ position_id, reason = "manual" }) {
  ...
  if (tracked.dry_run || isDryRun()) {
    ...
    return {
      ...
      position_id,
      reason,
      message: "DRY RUN close — state updated only",
    };
  }

  return {
    success: false,
    error: "Live close not fully wired — implement decreaseLiquidity + collect + burn in tools/univ3.js",
    position_id,
  };
}
...
export async function claimFees({ position_id }) {
  if (isDryRun()) {
    return { success: true, dry_run: true, position_id, fees_usd: 0, message: "DRY RUN claim" };
  }
  return { success: false, error: "Live claim not fully wired" };
}
